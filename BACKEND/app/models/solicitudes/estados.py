...
class Borrador(EstadoSolicitud):
  def nombre(self): return "BORRADOR"
  ...
  def agregar_items(self, solicitud, lista_items: list):
    ...
    if not all(type(item) is tipo_lote for item in lista_items):
      raise Exception("Error de Regla de Negocio: El listado enviado contiene una mezcla de Materiales y Servicios. Deben ser uniformes.")

    if solicitud.items:
      tipo_existente = type(solicitud.items[0])
      if tipo_lote is not tipo_existente:
        raise Exception(f"Error: La solicitud ya contiene items de tipo {tipo_existente.__name__} y intentas agregar {tipo_lote.__name__}.")

    solicitud.items.extend(lista_items)
    print(f"Agregados {len(lista_items)} items de tipo {tipo_lote.__name__}.")

  def aprobar(self, solicitud):
    if not solicitud.items:
      raise Exception("No se puede aprobar una solicitud sin items.")
    solicitud.set_estado(Pendiente())
